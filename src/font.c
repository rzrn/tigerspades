...
static void font_init_16px(GLint max_size) {
    if (settings.unicode_enabled) {
        ...
        gnubmp = upload_subfont("fonts/unifont/bmp.bitmap", max_size, 16);
        gnusmp = upload_subfont("fonts/unifont/smp.bitmap", max_size, 16);
        uvga   = upload_subfont("fonts/uvga/bmp.bitmap",    max_size, 16);

        static Subfont * primary_subfonts[]   = {&uvga, &gnubmp, &gnusmp};
        static Subfont * secondary_subfonts[] = {&gnubmp, &gnusmp};

        // Old GCC on 10.4 errors here if we replace `16` with the value of some `const uint16_t height`.
        ...
        gnuascii = upload_subfont("fonts/unifont/ascii.bitmap", max_size, 16);
        vga      = upload_subfont("fonts/uvga/ascii.bitmap",    max_size, 16);
...
static void font_init_24px(GLint max_size) {
    if (settings.unicode_enabled) {
        ...
        normal = upload_subfont("fonts/terminus-normal/bmp.bitmap", max_size, 24);
        bold   = upload_subfont("fonts/terminus-bold/bmp.bitmap",   max_size, 24);

        static Subfont * primary_subfonts[]   = {&bold};
        static Subfont * secondary_subfonts[] = {&normal};

        static Font primary   = {.replacement = 0xFFFD, .length = 1, .height = 24, .special = &bold,   .subfonts = primary_subfonts};
        static Font secondary = {.replacement = 0xFFFD, .length = 1, .height = 24, .special = &normal, .subfonts = secondary_subfonts};
        ...
        normal = upload_subfont("fonts/terminus-normal/ascii.bitmap", max_size, 24);
        bold   = upload_subfont("fonts/terminus-bold/ascii.bitmap",   max_size, 24);

        static Subfont * primary_subfonts[]   = {&bold};
        static Subfont * secondary_subfonts[] = {&normal};

        static Font primary   = {.replacement = 0x3F, .length = 1, .height = 24, .special = &bold,   .subfonts = primary_subfonts};
        static Font secondary = {.replacement = 0x3F, .length = 1, .height = 24, .special = &normal, .subfonts = secondary_subfonts};
