...
/* Tests if a position (p.x, p.x, p.z) plus an offset is within a given block (b[X], b[Y], b[Z]) */
#define SCLIP2(p, b, xoff, yoff, zoff) (floorf((p).x + (xoff)) == (b)[X] && \
                                        floorf((p).y + (yoff)) == (b)[Y] && \
                                        floorf((p).z + (zoff)) == (b)[Z])

/* Tests if a 0.9-wide plane xz-centered on a player (p.x, p.z), plus a Y offset, is within a given voxel (b[X], b[Y], b[Z]) */
#define SCLIPB2(p, b, yoff) (SCLIP2(p, b, -0.45, yoff, -0.45) || \
            ...
            while (mu_next_command(ctx, &cmd)) {
                switch (cmd->type) {
                    case MU_COMMAND_TEXT:
                        ...
                        font_draw_left(cmd->text.pos.x, settings.window_height - cmd->text.pos.y,
